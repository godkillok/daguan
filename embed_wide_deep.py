...
def build_model_columns():
    ...
    occupation = tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_hash_bucket(
        'occupation', hash_bucket_size=1000), 8,trainable=False)
...
def read_and_decode_tfrecords(filename_queue):
    ...
    examples = tf.parse_single_example(
        ...
        features={
            ...
            'income_bracket': tf.FixedLenFeature([], tf.string)# this different from version 3
        })
    ...
    label=tf.equal(batch_features['income_bracket'], '>50K')

    deep_features = tf.concat([tf.feature_column.input_layer(batch_features, deep_columns,trainable=True),
                               item2vec],1)
      ...
      try:
        while not coord.should_stop():
          if FLAGS.enable_benchmark:
            ...
            with open('C:/work/tensorflow_template/data/embeding/test.csv', 'a+') as abc:
                np.savetxt(abc, print_features, fmt='%.4f', delimiter=",")
