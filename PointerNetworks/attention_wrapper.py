...
class BahdanauAttention(_BaseAttentionMechanism):
  ...
  def __call__(self, query, previous_alignments):
    ...
    with variable_scope.variable_scope(None, "bahdanau_attention", [query]):
      processed_query = self.query_layer(query) if self.query_layer else query #just decode one time so the shape is(batch,150)
      # Reshape from [batch_size, ...] to [batch_size, 1, ...] for broadcasting.
      processed_query = array_ops.expand_dims(processed_query, 1)#the shape is(batch,1,150)
      keys = self._keys #(batch,senctenc,150)
      dtype = query.dtype
      ...
      if self._normalize:
        ...
        score = math_ops.reduce_sum(v * math_ops.tanh(keys + processed_query),
                                    [2]) #(batch size,sentence size)

    alignments = self._probability_fn(score, previous_alignments) #(batch size,sentence size)
...
############## DONE ####################################################
  ...
  def call(self, inputs, state):
    ...
    alignments, raw_scores = self._attention_mechanism(
        attention_input, previous_alignments=state.alignments) #alignments after softmax raw_scores(batch,sentence)

    expanded_alignments = array_ops.expand_dims(alignments, 1)

    attention_mechanism_values = self._attention_mechanism.values #.value is encoder after multply w. means  wh
    context = math_ops.matmul(expanded_alignments, attention_mechanism_values) #means how import per word in senctce
