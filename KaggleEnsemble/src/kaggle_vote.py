...
def kaggle_bag(glob_files, loc_outfile, method="average", weights="uniform"):
    ...
    for i in range(len(weight_list)):
        exp_weithgt[i] = math.exp(weight_list[i] / 1000)
    sum_exp = sum(exp_weithgt)
    # for i in range(len(weight_list)):
    #     exp_weithgt[i] = int((exp_weithgt[i] / sum_exp) * 1000)
    ...
    with open(loc_outfile, "w") as outfile:
        ...
        for i, glob_file in enumerate(glob(glob_files)):
            ...
            if weights == "weighted":
                ...
                weight_list[i]=int((weight_list[i] / sum_exp)*10)
                print("Using fixed weight: {}".format(weight_list[i]))
